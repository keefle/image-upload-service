...
>
  <div
    class="text-xl md:text-4xl mt-auto text-center font-black text-gray-600
      flex-none"
  >
  ...
  <button
    class:animate-pulse={percentCompleted !== -1}
  ...
  <div
    class:opacity-0={percentCompleted === -1}
    class="w-9/12 my-auto pt-1 transition-all duration-100"
  >
  ...
  <div
    class:opacity-0={serverResMsg === ""}
    class="transition-all duration-100
      text-center py-4 lg:px-4 w-3/4"
  >
    <div
      class="p-2 rounded-full bg-gray-300 items-center text-gray-600 leading-none lg:rounded-full flex lg:inline-flex"
      role="alert"
    >
      <span
        class:bg-red-300={serverResStat === "error"}
        class="flex rounded-full
      bg-green-400 uppercase px-2 py-1 text-xs font-bold mr-3"
      >
        {serverResStatMsg}
      </span>
      <span class="font-semibold mr-2 text-xs md:text-base text-left flex-auto"
        >{serverResMsg}</span
      >
    </div>
  </div>
